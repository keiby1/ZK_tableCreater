...
public class AppLayoutService {
    ...
    public String buildAppHeader() {
        return ""
                ...
                + "        var isServersPage = /\\/servers\\/?$/.test(path) && path.indexOf('serversJson') < 0;\n"
                + "        if (isServersPage) {\n"
                + "          if (jsonLink) jsonLink.setAttribute('href', '/serversJson' + q);\n"
                + "          if (htmlLink) htmlLink.style.display = 'none';\n"
                + "          if (excelLink) excelLink.style.display = 'none';\n"
                + "        } else {\n"
                + "          if (jsonLink) jsonLink.setAttribute('href', '/getJson' + q);\n"
                + "          if (htmlLink) { htmlLink.style.display = ''; htmlLink.setAttribute('href', '/getHtml' + q); }\n"
                + "          if (excelLink) { excelLink.style.display = ''; excelLink.setAttribute('href', '/getExcel' + q); }\n"
                + "        }\n"
                + "      })();\n"
                + "    </script>\n";
    }
